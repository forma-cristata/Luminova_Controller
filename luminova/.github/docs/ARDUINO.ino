...
// Flag to track if Still effect has been applied to prevent flickering
bool stillEffectSet = false;
...
void focalCheck(float delayTime) {
    static unsigned long lastFocalCheck = 0;
    handleWebServer();
    
    // Non-blocking delay using millis()
    if (millis() - lastFocalCheck < delayTime) {
        return; // Not enough time has passed, exit early
    }
    lastFocalCheck = millis();
    
    const byte HALL_PINS[] = { A0, A1, A2, A3, A4 }; 
    const int NUM_PINS = 5;

    const int BASE_READINGS[] = { 504, 510, 508, 502, 492 };
    ...
    // Log to prove focal checks are occurring
    Serial.print("FocalCheck: delay=");
    Serial.print(delayTime);
    Serial.print("ms, focal=");
...
void StateOfTrance() {
    ...
    if (focal == -1) {
        for (int j = 0; j < LIGHT_COUNT; j++) {
            focalCheck(delayTime * 4); // Check focal at start of each major loop
            for (int k = 0; k < sc1; k++) {
...
void Medusa() {
    if (focal == -1) 
    {
        for (int kc = 0; kc < LIGHT_COUNT; kc++) {
            focalCheck(delayTime * 2); // Check focal at start of each major loop
            for (int i = 0; i < LIGHT_COUNT; i++) {
...
void LapisLazuli() {
    ...
    if (focal == -1) {
        for (int i = 0; i < LIGHT_COUNT; i++) {
            focalCheck(delayTime * 2); // Check focal periodically during animation
...
void BerghainBitte() {
    ...
    if (focal == -1) {
        for (int i = 0; i < COLOR_COUNT; i++) {
            focalCheck(delayTime * 2); // Check focal at start of each color cycle
...
// 6 -> Still
void Still() {
    // Only set the LEDs if they haven't been set already or if configuration changed
    if (!stillEffectSet) {
        for (int i = 0; i < LIGHT_COUNT; i++) {
...
// 4 -> Mold
void Decay() {
    // TIMING FIX: Remove division by 4 to match React Native exactly
    for (int i = 0; i < LIGHT_COUNT; i++) {
        setLed(i, "#000000", 0, 0);
    }
    ...
    if (focal == -1) {
        for (int startIdx = LIGHT_COUNT - 1; startIdx >= 0; startIdx--) {
            focalCheck(delayTime); // Check focal periodically during animation
...
void FeelTheFunk() {
    ...
    for(int colorer = 0; colorer < COLOR_COUNT; colorer++) {
        focalCheck(delayTime * 2); // Check focal at start of each color cycle
        for (int strobe = 0; strobe < strobeCount1; strobe++) {
            if (strobe % 4 == 0) focalCheck(delayTime * 2); // Check focal every 4 strobes
            delay(delayTime * 12);
...
void ThePianoMan() {
    ...
    if (focal == -1) {
        for (int x = 0; x < COLOR_COUNT * 2; x++) {
            if (x % 4 == 0) focalCheck(delayTime * 2); // Check focal every 4 iterations
...
void StuckInABlender() {
    ...
    if (focal == -1) {
        focalCheck(delayTime); // Check focal at start
		for (int i = 0; i < LIGHT_COUNT; i++) {
            if (i % 4 == 0) focalCheck(delayTime); // Check focal every 4 LEDs
...
void selectEffect(int effectNumber) {
    // Reset Still effect flag when switching to any other effect
    if (effectNumber != 6) {
        stillEffectSet = false;
    }
...
void processJsonConfig(const String& jsonString) {
    ...
    if (doc.containsKey("effectNumber")) {
		if (effectNumber != doc["effectNumber"]) {
			...
			// Reset Still effect flag when effect changes
			stillEffectSet = false;
		}
    ...
    if (doc.containsKey("whiteValues")) {
        ...
        // Reset Still effect flag when white values change
        stillEffectSet = false;
    }
    ...
    if (doc.containsKey("brightnessValues")) {
        ...
        // Reset Still effect flag when brightness values change
        stillEffectSet = false;
    }
    ...
    if (doc.containsKey("colors")) {
        ...
        // Reset Still effect flag when colors change
        stillEffectSet = false;
    }
}
