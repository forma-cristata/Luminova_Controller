    ...
    const initializeColors = () => {
        if (setting.flashingPattern === "6") {
            ...
            for (let i = 0; i < LIGHT_COUNT; i++) {
                const colorIndex = i < setting.colors.length ? i : i % setting.colors.length;
        ...
        for (let colorer = 0; colorer < COLOR_COUNT; colorer++) {
            for (let strobe = 0; strobe < strobeCount1; strobe++) {
                ...
                for (let i = 0; i < ledsPerGroup; i++) {
                    ...
                    setLedColor((ledIndex + 1) % LIGHT_COUNT, setting.colors[(ledIndex+colorer) % COLOR_COUNT]);
                    await createTimeout(() => {}, setting.delayTime);
                }
        ...
        for (let startIdx = LIGHT_COUNT - 1; startIdx >= 0; startIdx--) {
            ...
            for (let strobe = 0; strobe < strobeCount1; strobe++) {
                ...
                for (let i = 0; i < ledsPerGroup; i++) {
                    ...
                    if (!isActive()) return;
                    setLedColor((ledIndex + 1) % LIGHT_COUNT, setting.colors[ledIndex % COLOR_COUNT]);
