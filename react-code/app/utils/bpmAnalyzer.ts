...
import {
	RecordingOptions,
	RecordingStatus,
} from "expo-audio/build/Audio.types";
...
export class BPMAnalyzer {
	...
	private checkForBeat() {
		...
		if (now - this.lastBeat > MIN_INTERVAL) {
			...
			this.beatTimes = this.beatTimes.filter(
...
// React component that provides BPM analysis functionality
const BPMAnalyzerComponent: React.FC<{
	onBPMUpdate?: (bpm: number) => void;
	onBPMStable?: (bpm: number) => void;
	stabilizationTime?: number;
	minimumBeats?: number;
	...
	useEffect(() => {
		analyzerRef.current = new BPMAnalyzer(props);
		return () => {
	...
	const stopAnalysis = async () => {
		try {
			...
			console.error("Failed to stop BPM analysis:", error);
		}
	};
...
export default BPMAnalyzerComponent;
