...
const Picker = forwardRef<PickerRef, PickerProps>(({ selectedPattern, setSelectedPattern }, ref) => {
			...
			<View style={styles.pickerContainer}>
				...
				>
					{FLASHING_PATTERNS.map(pattern => (
						<TouchableOpacity
							key={pattern.id}
							style={[styles.patternOption, selectedPattern === pattern.id && styles.selectedOption]}
							onPress={() => handlePatternSelect(pattern.id)}
						>
							<Text style={[styles.patternText, selectedPattern === pattern.id && styles.selectedText]}>
								{pattern.name}
							</Text>
						</TouchableOpacity>
					))}
